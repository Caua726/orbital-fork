export const DICT: Record<string, { pt: string; en: string }> = {
  ...
  'settings.weydra.tip_starfield': { pt: 'Estrelas geradas por shader (M2).', en: 'Stars rendered by shader (M2).' },
  'settings.weydra.tip_starfieldBright': { pt: 'Tiles aditivos de estrelas brilhantes (M3).', en: 'Additive bright star tiles (M3).' },
  'settings.weydra.tip_ships': { pt: 'Sprite pool weydra para todas as naves (M3).', en: 'Weydra sprite pool for every ship (M3).' },
  'settings.weydra.tip_shipTrails': { pt: 'Trails dos motores via sprite pool (M3).', en: 'Engine trails via sprite pool (M3).' },
  'settings.weydra.tip_planetsBaked': { pt: 'Planetas pequenos como sprite cacheado (M4).', en: 'Small planets as cached sprite (M4).' },
  ...
  'renderer_info.engine_weydra': { pt: 'Weydra ({n}/5 subsistemas)', en: 'Weydra ({n}/5 subsystems)' },
  ...
  'tooltips.weydraSubsystem': { pt: 'Subsistema do Weydra\n\nCada caixa controla um sistema gráfico portado pra\nweydra. Desabilite individualmente quando suspeitar\nque o port introduziu bug visual — o sistema cai de\nvolta pra implementação Pixi.\n\nMudanças exigem recarregar o jogo.', en: 'Weydra subsystem\n\nEach checkbox controls one graphics system ported\nto weydra. Toggle off individually when you\nsuspect the port introduced a visual bug — the\nsystem falls back to the Pixi implementation.\n\nChanges require reloading the game.' },
